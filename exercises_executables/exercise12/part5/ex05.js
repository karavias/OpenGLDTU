...
function render()
{
    ...
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "Projection"), false, new Float32Array(flatten(projection)));//(projectionUniform, 1, gl.TRUE, flatten(projection));
    gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "ModelView"), false, new Float32Array(flatten(modelView)));//(projectionUniform, 1, gl.TRUE, flatten(projection));
    ...
    gl.activeTexture(gl.TEXTURE0); // activate texture in slot 0
    gl.bindTexture(gl.TEXTURE_2D, texID); // assign texture to slot 0
    gl.uniform1i(gl.getUniformLocation(shaderProgram, "texture"), 0); // assign slot 0 to uniform
...
function bindAttributes()
{
    ...
    var uvLocation = gl.getAttribLocation(shaderProgram, "textureCoord");
...
function createVertexBufferObject(data)
{
    ...
    //alert(data.vertex.length + " -- " + data.normal.length + " -- " + data.uv1.length);

...
void display() {	
	...
	float scaleSize = sqrt(2) * 10;

	// todo transform modelView and draw triangles
	modelView =Scale(-10, 10, 0) *RotateZ(90);
	...
	modelView = Translate(-10, -10, 0) * Scale(-scaleSize, scaleSize, 0) * RotateZ(45);
	glUniformMatrix4fv(modelViewUniform, 1, GL_TRUE, modelView);
	glDrawArrays(GL_TRIANGLES, 0, NUMBER_OF_VERTICES);

	modelView = Translate(-10, -10, 0) * Scale(-scaleSize, -scaleSize, 0) * RotateZ(135);
	glUniformMatrix4fv(modelViewUniform, 1, GL_TRUE, modelView);
	glDrawArrays(GL_TRIANGLES, 0, NUMBER_OF_VERTICES);
	
	modelView = Translate(10, 10, 0) * Scale(-scaleSize, scaleSize, 0) * RotateZ(225);
	glUniformMatrix4fv(modelViewUniform, 1, GL_TRUE, modelView);
	glDrawArrays(GL_TRIANGLES, 0, NUMBER_OF_VERTICES);

	modelView = Translate(10, 10, 0) * Scale(-scaleSize, -scaleSize, 0) * RotateZ(315);
	glUniformMatrix4fv(modelViewUniform, 1, GL_TRUE, modelView);
	glDrawArrays(GL_TRIANGLES, 0, NUMBER_OF_VERTICES);

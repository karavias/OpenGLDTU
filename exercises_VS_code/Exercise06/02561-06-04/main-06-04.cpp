...
void buildProceduralTexture() {
	...
	for (int i = 0; i < checkImageWidth; i++) {
		for (int j = 0; j < checkImageHeight; j++) {
			...
			checkImage[i][j][1] = (GLubyte) (c*j/float(checkImageHeight-1));
			checkImage[i][j][2] = (GLubyte) (c*i/float(checkImageWidth-1));
		}
    }
...
void keyboard(unsigned char c, int x, int y){
	...
	case '1':
		...
		break;
	case '2':
		glutSetWindowTitle("T: Clamp");
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		break;
	case '3':
		glutSetWindowTitle("T: Mirror");
	...
	case '4':
		...
		break;
	case '5':
		glutSetWindowTitle("S: Clamp");
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		break;
	case '6':
		glutSetWindowTitle("S: Mirror");

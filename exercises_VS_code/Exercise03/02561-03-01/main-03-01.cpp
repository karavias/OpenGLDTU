...
vector<Vertex> interleaveData(vector<float> &position, vector<float> &color, vector<float> &position2, vector<float> &color2,
							vector<float> &normal1, vector<float> &normal2){
	...
	for (int i=0;i<position.size();i=i+3){
		...
		vec3 normalVec(normal1[i], normal1[i + 1], normal1[i + 2]);
...
void uploadData(vector<Vertex> &interleavedVertexData){
	...
	glEnableVertexAttribArray(normal1Attribute);
	...
	glVertexAttribPointer(normal1Attribute, 3, GL_FLOAT, GL_FALSE, stride, (const GLvoid *)(sizeof(vec3)* 4));
...
void loadShader(){
	...
	extrusionValueUniform = glGetUniformLocation(shaderProgram, "normalExtrusion");
	if (extrusionValueUniform == GL_INVALID_INDEX) {
	...
	normal1Attribute = glGetAttribLocation(shaderProgram, "normal1");
	if (normal1Attribute == GL_INVALID_INDEX){
...
void updateTitle(){
	...
	if (count == 10){
		char buffer[50];
		sprintf(buffer, "VertexBlend: Blend %.2f | NormalExtrusion : %.2f", blendValue, normalExtrusion);
...
void display() {	
	...
	glUniform1fv(extrusionValueUniform, 1, &normalExtrusion);

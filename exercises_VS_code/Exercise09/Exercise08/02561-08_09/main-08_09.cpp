...
void drawMeshObject(mat4 & projection, mat4 & model, mat4 & view, MeshObject& meshObject);

mat4 getLightProjection() {
	float dist = length(lightPosition - teapotPosition);
	float fov = RADIAN_TO_DEGREE * asin(teapotBoundingRadius / dist) * 2;
	return Perspective(fov, 1, 1, 400.0);
}

mat4 getLightView() {
	return LookAt(lightPosition, teapotPosition, vec4(1, 0, 0));
}
...
void updateProjShadowTexture() {
	...
	teapotObject.color = vec4(1, 1, 1, 1);
...
Shader loadShader(const char* vertShader, const char* fragShader) {
	...
	shader.shadowMapTextureUniform = glGetUniformLocation(shader.shaderProgram, "shadowTexture");
...
void drawMeshObject(mat4 & projection, mat4 & model, mat4 & view, MeshObject& meshObject) {
	...
	if (meshObject.shader.lightViewProjectionUniform != GL_INVALID_INDEX) {
		glUniformMatrix4fv(meshObject.shader.lightViewProjectionUniform, 1, GL_TRUE,
			getLightViewProjection());
	}
	if (meshObject.shader.shadowMapTextureUniform != GL_INVALID_INDEX) {
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, shadowmapTextureId);
		glUniform1i(meshObject.shader.shadowMapTextureUniform, 1);
	}
...
void drawMirror(mat4 &projection, mat4 &view) {
	...
	mat4 model = Translate(teapotPosition);
	mat4 mirroredView = view*Scale(1, -1, 1);
	teapotObject.color = vec4(0.5);
	drawMeshObject(projection, model, mirroredView, teapotObject);
	teapotObject.color = vec4(1);

...
void key2(){
	...
	vec3 w0(-eye.y, eye.x, 0);
	vec3 u0 = w0 * eye;
	float xAngle = atan2(dir.y, dir.x);
	float yAngle = asin(dir.z);
	//float zAngle = atan2(dot(w0, up)/dot(u0, up)/abs(w0) * abs(u0));
	//xAngle = xAngle * 180 / 3.14159265359;
	//zAngle = zAngle * 180 / 3.14159265359;

	//cout << xAngle << ", " << zAngle << endl;

//	float xAngle = dot(dir, vec3(1, 0, 0));
	//xAngle = xAngle / length(dir);
	//xAngle = acos(xAngle);
	//xAngle = xAngle * 180 / 3.14159265359;
	//float yAngle = dot(dir, vec3(0, 1, 0));
	//cout << dir<<" == " << xAngle << ", " << zAngle  << " , " << yAngle<< endl;
//	vec3 worldPos = at - eye;
	//view = Translate(worldPos)*RotateZ(zAngle) * RotateX(xAngle), RotateY(yAngle);
